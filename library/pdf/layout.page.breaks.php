...
class FootnoteLocation
{
    ...
    public function __construct($location, $content_height)
    {
...
class PageBreakLocation
{
    ...
    public function __construct($location, $penalty)
    {
        $this->location = round($location, 2);
        $this->penalty = $penalty;
    }

    public function _getFootnotesHeight($footnotes, $page_start, $location)
    {
        $i = 0;
    ...
    public function getPenalty($page_start, $max_page_height, $footnotes)
    {
     ...
     */
    public function _getPageBreakHeightPenalty($page_start, $max_page_height)
    {
...
{
    public static function _getBreakLocations(&$dom_tree)
    {
        $locations_ungrouped = self::_getPagesTraverse($dom_tree, 0);
        ...
        return self::sort_locations($locations_ungrouped);
    }

    public static function _getFootnotesTraverse(&$box)
    {
        $footnotes = array();
        ...
        if (is_a($box, 'BoxNoteCall')) {
            ...
            foreach ($box->content as $child) {
                $footnotes = array_merge($footnotes, self::_getFootnotesTraverse($child));
            }
        }
    ...
    public static function getPages(&$dom_tree, $max_page_height, $first_page_top)
    {
         ...
         */
        $footnotes = self::_getFootnotesTraverse($dom_tree);
        usort($footnotes, 'cmp_footnote_locations');

        $locations = self::_getBreakLocations($dom_tree);
    ...
    public static function _isForcedPageBreak($value)
    {
        return
    ...
    public static function _hasForcedPageBreakBefore(&$box)
    {
        return self::_isForcedPageBreak($box->getCSSProperty(CSS_PAGE_BREAK_BEFORE));
    }

    public static function _hasForcedPageBreakAfter(&$box)
    {
        return self::_isForcedPageBreak($box->getCSSProperty(CSS_PAGE_BREAK_AFTER));
    }

    public static function _getPagesTraverseBlock(&$box, &$next, &$previous, $penalty)
    {
        $locations = array();
         ...
         */
        if (self::_hasForcedPageBreakBefore($box)) {
         ...
         */
        if (self::_hasForcedPageBreakAfter($box)) {
         ...
         * Process box content
         */
        $locations = array_merge($locations, self::_getPagesTraverse($box, $penalty));

        return $locations;
    }

    public static function _getMoreBefore($base, $content, $size)
    {
        $i = $base;
        $more_before = 0;
    ...
    public static function _getMoreAfter($base, $content, $size)
    {
        $i = $base;
        $more = 0;
    ...
    public static function _getPagesTraverseTableRow(&$box, $penalty)
    {
        $locations = array();
        ...
        $cells = $box->getChildNodes();
        $null = null;
        $ungrouped_row_locations = self::_getPagesTraverseBlock($cells[0],
            $null,
            $null,
            $inside_penalty);
        $row_locations = self::sort_locations($ungrouped_row_locations);

        for ($i = 1, $size = count($cells); $i < $size; $i++) {
            $ungrouped_child_locations = self::_getPagesTraverseBlock($cells[$i],
                $null,
                $null,
                $inside_penalty);
            $child_locations = self::sort_locations($ungrouped_child_locations);
    ...
    public static function _getPagesTraverseInline(&$box, $penalty, $more_before, $more_after)
    {
        $locations = array();
    ...
    public static function &_getPrevious($index, $content, $size)
    {
        for ($i = $index - 1; $i >= 0; $i--) {
            $child = $content[$i];
            if (!$child->is_null()) {
    ...
    public static function &_getNext($index, &$content, $size)
    {
    ...
    public static function _getPagesTraverse(&$box, $penalty)
    {
        ...
        for ($i = 0, $content_size = count($box->content); $i < $content_size; $i++) {
            $previous_child =& self::_getPrevious($i, $box->content, $content_size);
            $next_child =& self::_getNext($i, $box->content, $content_size);
            $child =& $box->content[$i];
            ...
            if (is_a($child, 'BRBox')) {
                ...
                $locations = array_merge($locations, self::_getPagesTraverseBlock($child,
                    $next_child,
                    $previous_child,
                    $penalty));
            } elseif (is_a($child, 'TableCellBox')) {
                $null = null;
                $child_locations = self::_getPagesTraverseBlock($child, $null, $null, $penalty);
                ...
                if (is_a($previous_child, 'BRBox')) {
                    $more_before = self::_getMoreBefore($i, $box->content, $content_size);
                }

                if (is_a($next_child, 'BRBox')) {
                    $more_after = self::_getMoreAfter($i, $box->content, $content_size);
                }

                $locations = array_merge($locations, self::_getPagesTraverseInline($child, $penalty, $more_before, $more_after));
            } elseif (is_a($child, 'TableRowBox')) {
                $locations = array_merge($locations, self::_getPagesTraverseTableRow($child, $penalty));
            }
        }
    ...
    public static function sort_locations($locations_ungrouped)
    {
        if (count($locations_ungrouped) == 0) {
            return array();
        }
