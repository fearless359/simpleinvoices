...
class CSSValuePercentage extends CSSValue
{
    ...
    public function init($value, $status)
    {
        $this->_value = $value;
        $this->_status = $status;
    }

    public static function &_fromString($value, &$class_object)
    {
        if ($value == 'inherit') {
    ...
    public function units2pt($font_size)
    {
        if ($this->isNormal()) {
    ...
    public function getPoints($base_size = 0)
    {
