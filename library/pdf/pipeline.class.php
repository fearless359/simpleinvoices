...
*/
    ...
    public function &reflow_margin_boxes($page_no, &$media) {
        ...
        foreach ($boxes as $selector => $box) {
            $boxes[$selector]->reflow($this->output_driver, $media, $boxes);
        }
    ...
    public function _setupScales(&$media) {
        ...
        if ($g_config['scalepoints']) {
            $g_pt_scale = $g_px_scale * 1.33; // Leave this magic number alone or everything will explode!
        } else {
            $g_pt_scale = 1.0;
        }
    }
    ...
    private function &_layout_item($data_id, &$media, $offset, &$context, &$postponed_filter) {
        ...
        $this->_dispatcher->fire('after-parse',
                                 array('pipeline' => &$this,
                                       'document' => &$box,
                                       'media'    => $media));
