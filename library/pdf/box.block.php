...
class BlockBox extends GenericContainerBox {
     ...
     */
    public function &create_from_text($content, &$pipeline) {
        $box = new BlockBox();
        $box->readCSS($pipeline->getCurrentCSSState());
        $box->add_child(
                        InlineBox::create_from_text($content, $box->getCSSProperty(CSS_WHITE_SPACE), $pipeline));
        return $box;
    }
     ...
     *       definition to the GenericFormattedBox class, leaving only box-specific 'reflow_static' definitions in
     *       specific classes.
     *
     * @todo make relative positioning more CSS 2.1 compliant; currently, 'bottom' and 'right' CSS properties are
     *       ignored.
     *
    ...
    public function reflow(&$parent, &$context, $boxes=null) {
        switch ($this->getCSSProperty(CSS_POSITION)) {
            ...
            case POSITION_RELATIVE:
                 ...
                 * not re-positioned after B1's offset is applied. This implies that relative positioning may cause
                 * boxes
                 * to overlap. However, if relative positioning causes an 'overflow:auto' box to have overflow, the UA
                 * must
    ...
    /**
     * Reflow absolutely positioned block box.
     ...
     *
     * @link http://www.w3.org/TR/CSS21/visuren.html#dis-pos-flo CSS 2.1: Relationships between 'display', 'position',
     *       and 'float'
     */
    public function reflow_absolute(&$context) {
    ...
    /**
     * Reflow fixed-positioned block box.
     * Note that according to CSS 2.1
     * the only types of boxes which could be absolutely positioned are
     * 'block' and 'table'
     *
     ...
     */
    public function reflow_fixed(&$context) {
         ...
         * in future when we'll try to call get_..._width public functions for this box
         *
         * @todo Update the family of get_..._width public function so that they would apply constraint
        ...
        $this->put_full_width(
                        $wc->apply($this->get_width(), $containing_block['right'] - $containing_block['left']));
     ...
     * @see GenericContainerBox
     */
    public function reflow_static(&$parent, &$context) {
     ...
     * @see GenericContainerBox
     */
    public function reflow_static_normal(&$parent, &$context) {
        ...
        if ($parent) {
             ...
             *
             * @link http://www.w3.org/TR/CSS21/visudet.html#blockwidth CSS 2.1. 10.3.3 Block-level, non-replaced
             ...
             * Unlike tables, DIV width is either constrained
             * by some CSS rules or expanded to the parent width; thus, we can calculate 'auto' margin
             * values immediately.
             *
             * @link http://www.w3.org/TR/CSS21/visudet.html#Computing_widths_and_margins CSS 2.1 Calculating widths and
             *       margins
             */
            ...
            /**
             * Now, place the current box upper left content corner.
             * Note that we should not
         ...
         * use 'get_first' public function instead of just accessing the $content array.
         *
    ...
    public function show(&$driver) {
        if ($this->getCSSProperty(CSS_FLOAT) != FLOAT_NONE || $this->getCSSProperty(CSS_POSITION) == POSITION_RELATIVE) {
        ...
        if ($position == POSITION_FIXED) {
            ...
            if (!$top->isAuto()) {
                ...
                if ($bottom->isPercentage()) {
                    $vertical_offset = $driver->getPageMaxHeight() * (100 - $bottom->getPercentage()) / 100 -
                                     $this->get_height();
                } else {
