...
class InlineBox extends GenericInlineBox {
    ...
    public function &get_line_box($index) {
        $line_box = & $this->_lines[$index];
        return $line_box;
    }

    public function get_line_box_count() {
        return count($this->_lines);
    }

    // Inherited from GenericFormattedBox
    public function process_word($raw_content, &$pipeline) {
        if ($raw_content === '') {
            return false;
        }
            ...
            if ($char == SYMBOL_SHY) {
                ...
                if (is_null($mapping)) {
                    ...
                    if (isset($mapping[$encoding])) {
                        ...
                        // character, an () - text box with no letters will be generated, in rare case causing a random
                        // line
    ...
    //
    public function init_line(&$box, &$line_no) {
    ...
    // @param $line_no number of line box
    //
    public function extend_line(&$box, $line_no) {
    ...
    public function merge_line(&$box, $line_no) {
        $start_line = 0;
     ...
     * Deprecated
     */
    public function getLineBoxCount() {
        return $this->get_line_box_count();
    }

    public function &getLineBox($index) {
