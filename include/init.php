...
global $apiRequest, $authSession, $config, $module, $pdoDbAdmin, $view;
...
try {
    $authSession->setExpirationSeconds($timeout * 60);
} catch (Zend_Session_Exception $zse) {
    SiError::out('generic', 'Zend_Session_Exception', $zse->getMessage());
}

$frontendOptions = ['lifetime' => $timeout * 60, 'automatic_serialization' => true];
 ...
 * *************************************************************/
$backendOptions = ['cache_dir' => './tmp/cache']; // Directory where to put the cache files
...
try {
    $smarty->registerPlugin('modifier', "siLocal_number"         , ["Inc\Claz\SiLocal", "number"]);
    $smarty->registerPlugin('modifier', "siLocal_number_trim"    , ["Inc\Claz\SiLocal", "numberTrim"]);
    $smarty->registerPlugin('modifier', "siLocal_currency"       , ["Inc\Claz\SiLocal", "currency"]);
    $smarty->registerPlugin('modifier', "siLocal_date"           , ["Inc\Claz\SiLocal", "date"]);
    $smarty->registerPlugin('modifier', "siLocal_truncateStr"    , ["Inc\Claz\SiLocal", "truncateStr"]);
    $smarty->registerPlugin('modifier', "siLocal_sqlDataWithTime", ["Inc\Claz\SiLocal", "sqlDateWithTime"]);
...
if (!$apiRequest) {
    Log::out("init.php - authentication-enabled[{$config->authentication->enabled}] fake_auth[{$authSession->fake_auth}]", Zend_Log::DEBUG);
    ...
    if ($config->authentication->enabled == ENABLED && $authSession->fake_auth == "1") {
        try {
            Zend_Session::start();
    ...
    if ($config->authentication->enabled == ENABLED) {
        ...
        if ($authSession->domain_id == null) {
            $authSession->domain_id = "1";
        }
        ApiAuth::authenticate($module, $authSession);
    } else {
        ...
        $authSession->email = "demo@simpleinvoices.group";
        // fake_auth is identifier to say that user logged in with auth off
        $authSession->fake_auth = "1";
