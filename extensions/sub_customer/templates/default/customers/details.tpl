    ...
    <div class="si_form si_form_view" id="si_form_cust">
        <div id="tabs_customer">
            <ul class="anchors">
                ...
                <li {if $sub_customer_count == 0}style="display:none"{/if}><a href="#section-6" target="_top">{$LANG.sub_customers}</a></li>
            </ul>
                        ...
                        <select name="default_invoice" tabindex="165">
                            ...
                            {foreach $invoices as $invoice}
                                <option {if $invoice.index_id == $customer.default_invoice}selected{/if}
                                        value="{$invoice.index_id}">Inv#{$invoice.index_id} ({$invoice.customer|htmlsafe} {$invoice.total|siLocal_number})</option>
                            {/foreach}
                        </select>
