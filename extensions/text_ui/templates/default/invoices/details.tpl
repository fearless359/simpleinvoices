  ...
  <table class="center">
    ...
    <tr>
      <td class="details_screen">
        {$preference.pref_inv_wording}&nbsp;{$LANG.number_short}
      </td>
      <td>
        <input type="hidden" name="invoice_id" value={$invoice.id} size="15" />
        {$invoice.id}
      </td>
    </tr>
    <tr>
      ...
      <td>
        <input type="text" class="date-picker" name="date" id="date1" value='{$invoice.calc_date}' />
      </td>
    </tr>
    <tr>
      ...
      <td>
        ...
        {else}
          ...
          {foreach from=$billers item=biller}
            <option {if $biller.id == $invoice.biller_id} selected {/if} value="{$biller.id}">
              {$biller.name}
            </option>
          {/foreach}
          </select>
    ...
    <tr>
      ...
      <td>{if $customers == null}
        <p>
          <em>{$LANG.no_customers}</em>
        </p> {else} <select name="customer_id"> {foreach
          from=$customers item=customer}
          <option {if $customer.id==
            $invoice.customer_id} selected {/if} value="{$customer.id}">{$customer.name}</option>
          {/foreach}
      </select> {/if}
      </td>
    </tr>
      ...
      <td colspan="6">
        <table>
          ...
          <tr>
            ...
            {else}
              <select class="product_select{$line} selector" name="products{$line}" 
                ...
                <option {if $product.id == $invoiceItem.product_id} selected {/if} value="{$product.id}">
                  {$product.description}
                </option>
                {/foreach}
              </select>
            ...
            <td>
              <select id="attr3-{$line}" name="attr3-{$line}" class="linkSel">
                <option value="{$invoiceItem.attr3.id}">
                  {$invoiceItem.attr3.display}
                </option>
                {foreach from=$invoiceItem.attr_all_3 item=invoiceItemAll3}
                <option value="{$invoiceItemAll3.id}">
                  {$invoiceItemAll3.display}
                </option>
                {/foreach}
              </select>
            </td>
    ...
    <tr>
      ...
      <td>
        ...
        <select name="tax_id">
          ...
          <option {if $tax.tax_id == $invoiceItems.0.tax_id} selected {/if}
                  value="{$tax.tax_id}">{$tax.tax_description}
          </option>
          {/foreach}
        </select>
        {/if}
      ...
      <td> 
        ...
        <select name="preference_id">
          ...
          <option {if $preference.pref_id == $invoice.preference_id} selected {/if}
                  value="{$preference.pref_id}">
            {$preference.pref_description}
          </option>
          {/foreach}
        </select>
        {/if}
