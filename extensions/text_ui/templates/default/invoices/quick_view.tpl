...
{$LANG.actions}: 
<a href="index.php?module=invoices&amp;view=details&amp;invoice={$invoice.id}&amp;action=view">
  {$LANG.edit}
</a>
::
<a href="index.php?module=payments&amp;view=process&amp;invoice={$invoice.id}&amp;op=pay_selected_invoice">
  {$LANG.process_payment}
</a>
::
<a href="index.php?module=invoices&amp;view=email&amp;stage=1&amp;invoice={$invoice.id}">
  {$LANG.email}
</a>
{if $defaults.delete == '1'} 
  :: 
  <a href="index.php?module=invoices&amp;view=delete&amp;stage=1&amp;invoice={$invoice.id}">
    {$LANG.delete}
  </a>
{/if}
<!--Actions heading - start-->
...
<b>{$preference.pref_inv_wording} {$LANG.summary}:</b>
<br />
...
<br />
<!-- Biller section -->
<b>{$LANG.biller}:</b>XXXXX {$biller.name}<br />
      ...
      <td colspan="6">
        <table style="width:100%;"> 
          {if $invoice.type_id == 2 || $invoice.type_id == 4}
            <tr>
              <td colspan="6" class="details_screen align_right">
                <a href='#' class="show-itemised" onclick="javascript: $('.itemised').show();$('.show-itemised').hide();">{$LANG.show_details}</a><a href='#' class="itemised" onclick="javascript: $('.itemised').hide();$('.show-itemised').show();">
                  {$LANG.hide_details}
                </a>
              </td>
            </tr>
          ...
          {if $invoice.type_id == 3 }
            <tr>
              <td colspan="6" class="details_screen align_right">
                <a href='#' class="show-consulting" onclick="javascript: $('.consulting').show();$('.show-consulting').hide();">{$LANG.show_details}</a><a href='#' class="consulting" onclick="javascript: $('.consulting').hide();$('.show-consulting').show();">
                  {$LANG.hide_details}
                </a>
              </td>
            </tr>
            ...
            {if $invoice.type_id == 2 || $invoice.type_id == 4}
              <tr>
                ...
                <td colspan="3">
                  ...
                  {$invoiceItem.attr1.display}
                  {$invoiceItem.attr2.display} {$invoiceItem.attr3.display}
                </td>
              </tr>
              <tr>
            ...
            {if $invoice.type_id == 3 }
              ...
              {if $invoiceItem.description != null}
                <tr class='show-consulting' >	
                  <td colspan="6" class='details_screen consulting'>
                    {$invoiceItem.description|truncate:"..."}
                  </td>
                </tr>
                <tr class='consulting' >	
                  <td colspan="6" class='details_screen consulting'>
                    {$LANG.description}:<br />{$invoiceItem.description}
                  </td>
                </tr>
              {/if}
            {/if}
  ...
  <table>
    <tr>
      <td class="details_screen align_right">
        <a href='#' class="show-notes" onclick="javascript: $('.notes').show();$('.show-notes').hide();">
          {$LANG.show_details}
        </a>
        <a href='#' class="notes" onclick="javascript: $('.notes').hide();$('.show-notes').show();">
          {$LANG.hide_details}
        </a>
      </td>
    </tr>
